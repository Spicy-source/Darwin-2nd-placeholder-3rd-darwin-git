...
const SOURCE_DECISION = resolve(SOURCE, "..", "..", "hf-genome-confirmation-002.json");
...
test("portable offline evidence reconstructs with zero provider calls", () => {
  ...
  try {
    ...
    assert.ok(
      verified.result.metrics.candidate_holdout_accuracy <=
        verified.result.metrics.baseline_holdout_accuracy,
    );
...
test("source, listing and prediction forgeries fail closed", () => {
  reject(
    ({ root }) =>
      rewriteJson(join(root, "input.json"), (input) => {
        delete input.source.output_evidence.evidence_root_sha256;
      }),
    "INPUT_HASH_DRIFT",
  );
  reject(
    ({ root }) =>
      rewriteJson(join(root, "source-population.json"), (source) => {
        source.rows.find((row) => row.kind === "predictions_committed").population_weights[
          "genome:821ca59d4e0be0e3"
        ].age = 9;
      }),
